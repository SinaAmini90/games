...
import { dimondLengthCal, posibleCellScoring, cpGuess, equalMode } from "./gameMechanics.js";
...
function storGameBoard() {
    ...
    localStorage.setItem('fulledCellQuantitySaved', `${JSON.stringify(fulledCellQuantity)}`);
}
function loadGameBoard() {
    let matrixBoardLoaded = localStorage.getItem('matrixBoardSaved');
    let fulledCellQuantityLoaded = localStorage.getItem('fulledCellQuantitySaved');
        ...
        for (let i = 0; i < matrixBoard.length; i++) {
            for (let j = 0; j < matrixBoard[i].length; j++) {
                if (matrixBoard[i][j] == 'cpD') {
                    ...
                    document.querySelector(`.cell[data-x="${i}"][data-y="${j}"]`).appendChild(downRowImage);
    ...
    async function fallingTopRow() {
        if (player1.turn) {
            ...
            if (updateFulledCellQuantity(xCell)) {
                ...
                await delay(900);
                document.querySelector(`.cell[data-x="${xCell}"][data-y="${yCell}"]`).appendChild(downRowImage);
...
export { xCell, yCell, matrixBoard, fulledCellQuantity, player1, player2, playerTurnOnBoard };